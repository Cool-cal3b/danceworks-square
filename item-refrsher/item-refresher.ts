...
export default class ItemRefresher {
    ...
    private _squareItemToAirtableRecord(item: SquareItem) {
        ...
        if (item.defaultUnitCost) {
            ...
            if (!Number.isNaN(cost)) fields['Default Unit Cost'] = cost;
        }
        if (item.defaultVendorName) fields['Default Vendor Name'] = item.defaultVendorName;
        if (item.defaultVendorCode) fields['Default Vendor Code'] = item.defaultVendorCode;
